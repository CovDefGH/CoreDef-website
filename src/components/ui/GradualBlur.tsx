...
function GradualBlur(props: any) {
  ...
  const config = useMemo(() => {
    const presetConfig =
      props.preset && (PRESETS as any)[props.preset]
        ? (PRESETS as any)[props.preset]
        : {};
  ...
  const blurDivs = useMemo(() => {
    ...
    const curveFunc =
      (CURVE_FUNCTIONS as any)[config.curve] || CURVE_FUNCTIONS.linear;
    ...
    for (let i = 0; i < config.divCount; i++) {
      ...
      divs.push(<div key={i} style={divStyle as any} />);
    }
